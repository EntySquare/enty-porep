...
impl<Tree: MerkleTreeTrait, G: 'static + Hasher> Proof<Tree, G> {
    ...
    pub fn synthesize<CS: ConstraintSystem<Fr>>(
        ...
        debug!("synthesize start : challenge= {:?}", challenge.unwrap());
        ...
        debug!("gen data_leaf_num end duration:{:?}", now.elapsed());
        ...
        let now = Instant::now();
        debug!(" gen drg_parents start.. layers = {}", layers);
        ...
        debug!(" gen drg_parents end duration:{:?}", now.elapsed());

        // Private Inputs for the Expander parent nodes.
        let now = Instant::now();
        debug!(" gen exp_parents start..");
        ...
        debug!(" exp_parents length = {}", exp_parents.len());
        debug!(" gen exp_parents end duration:{:?}", now.elapsed());
        // -- Verify labeling and encoding
        let now = Instant::now();
        debug!(" gen column_labels start..");
        ...
        for layer in 1..=layers {
            ...
            trace!("layer[{}] collect the parents : drg_parents len:{}, exp_parents:{}", layer, drg_parents.len(), exp_parents.len());
            trace!("layer[{}] collect the parents end duration:{:?}", layer, par_now.elapsed());

            let par_now = Instant::now();
            trace!("layer[{}] expanded_parents start..", layer);
            ...
            trace!("layer[{}] expanded_parents : parents len:{}, expanded_parents:{}", layer, parents.len(), expanded_parents.len());
            trace!("layer[{}] expanded_parents end duration:{:?}", layer, par_now.elapsed());

            let par_now = Instant::now();
            trace!("layer[{}] create_label_circuit start..", layer);
            ...
            trace!("layer[{}] create_label_circuit end duration:{:?}", layer, par_now.elapsed());
            column_labels.push(label);
            trace!("layer[{}] create label end duration:{:?}", layer, par_start.elapsed());
        }
        debug!(" column_labels length = {}", column_labels.len());
        debug!(" gen column_labels end duration:{:?}", now.elapsed());
        // -- encoding node
        {
            // encode the node
